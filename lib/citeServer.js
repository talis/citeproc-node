 ...
 * It has been superseded by ./citeServerLambda.js which runs on AWS Lambda via AWS API Gateway
 * Left in here so that the existing citeproc server can still run until we move consuming applications
 *   over to the new API Gateway endpoint. Once everything has moved, we should remove this file to
 *   avoid any confusion over what is running.
 */
